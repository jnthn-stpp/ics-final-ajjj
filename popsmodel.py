...
class Env(object):
    ...
    def populate(self, pop=None, rows=5, pos=None):
        # Add a population to the environment
        ...
        if pos is None:
            ...
            for i in range(pop.n):
                ...
                if not self.grid[r][c]:
                    self.grid[r][c] = pop.agents[i]
    def depopulate(self, pop=None):
        # Dissociate population from environment
        # Does not affect population or its agents
        self.clear("agents", pop)
        if pop is None:
            self.pop.clear(); return
        return self.pop.pop(self.pop.index(pop))
            ...
            for c in range(2, self.width + 2):
                if w is "all":
                    ...
                    if type(self.grid[r][c]) is Agent:
                        if pop is None or self.grid[r][c] in pop.agents:
                            self.grid[r][c] = False
    ...
    def extend(self, rows=1, blocking=False, density=0.05):
        ...
        self.length += rows
    def find(self, identity, pop=None):
            ...
            for c in range(2, self.width + 2):
                if type(self.grid[r][c]) is Agent:
                    if self.grid[r][c].i == identity:
                        if self.grid[r][c] in pop.agents:
                            return (r, c)
        print("Agent not found")
    def show(self, identity, pop=None):
        # Display agent by ID
        if pop is None:
            pop = self.pop[-1]
        out = ""
        for i in self.grid:
            for j in i:
                ...
                elif j.i == identity and j in pop.agents: s = "@"
                else: s = "-"
                out += s
            out += "\n"
        print(out[:-1])
    def popmap(self, pop=None):
        out = ""
        if pop is None:
            for i in self.grid:
                for j in i:
            ...
            for i in self.grid:
                for j in i:
                    ...
                    elif j in pop.agents: s = "O"
                    else: s = "-"
                    out += s
                out += "\n"
        print(out[:-1])
    def remove(self, identity, pop=None):
        # Remove agent by ID
        # Does not remove agent from its population
        r, c = self.find(identity, pop)
        self.grid[r][c] = False
    def neighborhood(self, identity=None, pop=None, r=None, c=None, index=False):
